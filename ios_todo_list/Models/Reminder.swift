...
/*
 フラグは、#if DEBUGリリース用にアプリをビルドするときに、
 同封のコードがコンパイルされないようにするコンパイル ディレクティブです。
 このフラグを使用して、デバッグ ビルドでコードをテストしたり、
 次の手順で行うようにサンプル テスト データを提供したりできます。
 */

//サンプル リマインダー データを保存する拡張機能
// テストデータが入ってるやつ。うれしい。
#if DEBUG
extension Reminder {
